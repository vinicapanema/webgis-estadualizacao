...
document.addEventListener('DOMContentLoaded', () => {
  const geoServerBase = window.geoServerBase;
    ...
    for (const [title, cfg] of Object.entries(config)) {
      if (cfg.layer) {
        ...
        checkbox.addEventListener('change', () => {
          if (checkbox.checked) {
            if (cfg.type === 'wfs') {
              const wfsUrl = `${geoServerBase}/wfs?service=WFS&version=1.0.0&request=GetFeature&typename=${cfg.layer}&outputFormat=application/json`;
              fetch(wfsUrl)
                .then(r => r.json())
                .then(data => {
        ...
        // ✅ Ativa automaticamente a camada "Limite Estadual"
        ...
        // Caixa do grupo controla todas as camadas internas
